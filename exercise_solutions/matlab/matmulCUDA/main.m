...
gpukernel = parallel.gpu.CUDAKernel( 'kernel.ptx', 'kernel.cu', 'GPU_matmul' );

threads_X = 16;
threads_Y = 16;
gpukernel.ThreadBlockSize = [ threads_X, threads_Y, 1 ];
gpukernel.GridSize = [ size/threads_X, size/threads_Y ];
...
d_B = gpuArray(B);
t2 = toc(t1);
d_C = feval( gpukernel, d_C, size, d_A, d_B );
wait(g);
t3 = toc(t1);
h_C = gather( d_C );
gpuT = toc(t1);
